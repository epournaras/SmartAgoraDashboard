...
$(document).ready(function () {
    ...
    $(".chk-number-input").on('keyup focusout', function () {
        if ((event.keyCode !== 13 || event.which !== 13) && (isNaN(parseInt($(this).val())) || parseInt($(this).val()) == 0)) {
            ...
            var id = $(this).prop("id");
            if (id === "defaultCredit")
                $(".buttonNext").addClass("buttonhide");
            $("#" + id).val("");
        }
        else {
...
function saveValuesOnNextButton() {
    if ($("#currentScreen").val() === "projectAndMode") {
        saveProjectIdAndAssetMode();
        setStartAndDestinationStep(startAndDestinationModal.Mode);
        $("#currentScreen").val("startAndDestination");
        return true;
    }
    else if ($("#currentScreen").val() === "startAndDestination") {
        var r = saveStartAndDestinationData();
        if (r)
            $("#currentScreen").val("Question");
        return r;
    }
...
function setStartAndDestinationStep() {
    ...
    if (assetMode === "Sequence" || assetMode === "Decision") {
    ...
    else if (assetMode === "Simple" || assetMode === "DIAS_Simple") {
        $("#startAddressDiv").hide();
        $("#destinationAddressDiv").hide();
        $("#googleMap").addClass("disabledMap");
    }
...
function hideGoogleMap() {
    ...
    if ($("#currentScreen").val() === "Question") {
        ...
        $("#currentScreen").val("projectAndMode");
    }
}
...
$(document).on("change", "#questionType", function () {
    ...
    if (questionType === "radio") {
        $("#addButton").removeClass("buttonhide");
...
$(document).ready(function() {
    ...
    $(document).ready(function() {
    $('#optQuestion' + optionCounter).on('input', function() {
        var input = $(this);
        var is_name = input.val();
        
        if(is_name){
            $('#optQuestion' + optionCounter).removeClass("parsley-error");
        }
        else{
            $('#optQuestion' + optionCounter).addClass("parsley-error");
        }
    });
...
function addQuestionInList(qId) {
    ...
    //for(var qn = 0; qn<lstQuestionDataModel.length; qn++) {
    //    if(lstQuestionDataModel[qn].Option[qn].Name === $("#txtquestion").val()) {
    //        msg += "=> same Question already exist</br>";
    //    }
    //}
    ...
    if ($("#txtquestion").val() === "") {
        msg += "=> Question</br>";
        $("#txtquestion").addClass("parsley-error");
        }
        
        
    if (questionLatitude === null && questionLongitude === null && !updateSts){
         msg += "=>Please mark question location on Map.</br>";
         $("#questionAddress").addClass("parsley-error");
    } else {
        $("#questionAddress").removeClass("parsley-error");
    }
    ...
    if (updateSts) {
        ...
        var temp = new google.maps.Marker({
            position: new google.maps.LatLng(questionLatitude, questionLongitude),
            map: map,
            label: { text: objQuestionId + "", color: "white", fontSize: "15px", fontWeight: "bold" },
            icon: '/Content/img/blue-dot.png'
        });
        if ($('#assetMode').val() === "DIAS_Simple") {
...
function optionAddRemoveButtonHtml(rmAddButtonCount) {
    var customHtml = '<div class="classRemoveOption form-control form-control-option-deleteBtn" id="removeOption-' + rmAddButtonCount + '"><i class = "fa fa-remove"></i></div>';
    return customHtml;
}
